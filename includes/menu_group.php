<?php
	...
	class Menu_group extends DatabaseObject {
	protected static $table_name="menu_group";
	protected static $db_fields=array('id','title');
	protected static $db_value=array(':id'=>'id',':title'=>'title');
	...
	public function find_in_not($id = '')
    {
        try
        {
            global $database;
            $sql = "SELECT * FROM " . self::$table_name . " WHERE `id` NOT IN (".$id.") ";
            $sql = $database->prepare($sql);
            $x = $sql->execute();
            self::fetch_mode($sql);
            $result = $sql->fetchall(PDO::FETCH_OBJ);
