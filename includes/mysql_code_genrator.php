...
class Mysql_code_genrator extends DatabaseObject {
   ...
   global $database;
$q = $database->prepare("DESCRIBE $table");
$q->execute();
$table_fields = $q->fetchAll(PDO::FETCH_COLUMN);
$total=count($table_fields);
for ($counter = 0; $counter < $total; $counter ++) { 
$fieldnames[]= $table_fields[$counter];
$fls[]= ":".$table_fields[$counter]."'=>'".$table_fields[$counter];
}
	...
	';
$x1.="protected static $".'db_value=array('.$xf.');
	';
  ...
  //Genrate Class
	...
	if (file_exists($path)) 
	{
	echo "<br><span style='color:red;font-weight:bold;'>".ucfirst($table)."</span> <strong>Class </strong> <strong>already exists: </strong>".$path."<br>";
	}
	else
	{
...
fclose($myfile);
  ...
  ";	
  $xp='';
  $paths="includes/temp/model_function.txt";
  $myfile = fopen("$paths", "r") or die("Unable to open file!");
	while(!feof($myfile)) {
	$xp.=fgets($myfile);
	}
	fclose($myfile);
  ...
  $x1.="
	...
	{
		echo "<br><strong>The Model File of <span style='color:red'>".ucfirst($table)."</span> :</strong> <span style='color:red'>".$path."</span> <strong>already exists</strong>";
		}
	}
