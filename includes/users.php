<?php
	...
	protected static $db_fields=array('id','adm_username','adm_password','adm_image','adm_fullname','adm_email','adm_phone','role','about','created','updated','status');
	protected static $db_value=array(':id'=>'id',':adm_username'=>'adm_username',':adm_password'=>'adm_password',':adm_image'=>'adm_image',':adm_fullname'=>'adm_fullname',':adm_email'=>'adm_email',':adm_phone'=>'adm_phone',':role'=>'role',':about'=>'about',':created'=>'created',':updated'=>'updated',':status'=>'status');
...
//  $encname=$date; /$imgname=md5($encname).'.'.$n[0].'.'.$n[1];
    ...
    $username = $username;
    $password = $password;
	$cs=self::count_by_x("adm_username",$username);
	if($cs==1)
	{
		
	$xx=self::find_by_x("adm_username",$username);
	}else
	{
    ...
    $sql .= " WHERE adm_username = ? ";
    $sql .= "AND adm_password = ? ";
    $sql .= "LIMIT 1";
	$sql = $database->prepare($sql);
    ...
    public function find_by_id($id)
    {
	 ...
	 public function find_by_x($field = '', $value = 0)
    {
        try
        {
            global $database;
