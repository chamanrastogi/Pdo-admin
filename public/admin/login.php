...
<?php	
    ...
    $type='danger';
	$message = $found_user;
	//redirect_by_js("users/login",1000,$type,$message);
  }
...
<div id="single-wrapper">
             ...
             <?php if($message!='')
  {
	  ...
	  echo'<div class="alert alert-'.$type.'">
                '.$message.'</div><br><center><img src="'.TP_BACK.'assets/loaders/loader_ge.gif" title="c_loader_re.gif"></center>';
	  } 
	 ?>
			<!-- /.title -->
          ...
          <label for="inputName" class="control-label">Username:(<span class="error">*</span>)</label>
          <input type="text" class="form-control" id="inputName" name="username" placeholder="Username" value="<?=htmlentities($username); ?>" required>
        </div>
        
        <div class="form-group">
          <label for="inputName" class="control-label">Password:(<span class="error">*</span>)</label>
          <input type="password"  maxlength="12"  placeholder="Password" name="password" value="<?=htmlentities($password); ?>" required class="form-control">
			...
			<div class="clearfix margin-bottom-20">
				...
				<!-- /.pull-left -->
				<div class="pull-right"><a href="<?=TP_BACK."admin/forget-password"?>" class="a-link"><i class="fa fa-unlock-alt"></i>Forgot password?</a></div>
				<!-- /.pull-right -->
			</div>
			<!-- /.clearfix -->
