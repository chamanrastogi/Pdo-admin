...
class Menu extends GController {
		...
		if (isset($_POST['title'])) {
			...
			if (!empty($data[MENU_TITLE])) {
				...
				if ($this->db->update(MENU_TABLE, $data, MENU_ID . ' = ' . $data[MENU_ID])) {
					...
					$d['url'] = $data[MENU_URL];
					$d['class'] = $data[MENU_CLASS];
					$d['status'] = $data[MENU_STATUS]; //klass instead of class because of an error in js
					$response['menu'] = $d;
				} else {
					$response['status'] = 2;
	...
	private function get_label($row) {		
		$label =
			'<div class="ns-row">' .
				'<div class="ns-title">'.ucfirst($row[MENU_TITLE]).'</div>'.
				'<div class="ns-url">'.$row[MENU_URL].'</div>'.
				'<div class="ns-class"><a href="menu.php?acts='.$row[MENU_ID].'&act=menu">'.$row[MENU_STATUS].'</div>'.
				'<div class="ns-actions">' .
					'<a href="'._BASE_URL.'menus/edit/'.$row[MENU_ID].'"  title="Edit Menu">' .
