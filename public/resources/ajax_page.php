...
if( !empty($requestData['search']['value']) ) {
	...
	$sql.=" OR mid =(SELECT id FROM `menus` WHERE title='".strtolower($requestData['search']['value'])."')";    	
	$sql.="  )";	
}
...
$ro=$table::find_by_id($row['id']);
$mu=Menus::find_by_id($row['mid']);
...
$code=TP_BACK_SIDE.$url.'/delete/'.$ro->id;			
$nestedData[] = $ro->id;
$nestedData[] = $row['mid'];
...
$nestedData[] = '<a href="'.TP_BACK_SIDE.$url.'/edit/'.$ro->id.'" class="btn btn-info btn-circle btn-xs waves-effect waves-light">
				       <i class="ico fa fa-pencil "></i></a>'.$st."'$code'".$end;
